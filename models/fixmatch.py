...
def choose_network(args):
    ...
    if args.net_from_name:
        import torchvision.models as models
        model_name_list = sorted(name for name in models.__dict__
                                if name.islower() and not name.startswith("__")
                                and callable(models.__dict__[name]))

        if args.net not in model_name_list:
            assert Exception(f"[!] Networks\' Name is wrong, check net config, \
                               expected: {model_name_list}  \
                               received: {args.net}")
        else:
            net_model = models.__dict__[args.net]
            if args.pretrained_from == 'scratch':
                model = net_model(pretrained = False, num_classes=10)
                model.fc = nn.Linear(model.fc.in_features, 10)

            elif args.pretrained_from == 'ImageNet_supervised':
                model = net_model(pretrained = True)
                model.fc = nn.Linear(model.fc.in_features, 10)
            elif "SimCLR" in args.pretrained_from :
                model = models.resnet50(pretrained=False, num_classes=10)
                dataset = args.pretrained_from.split('_')[0]
                checkpoint_dir = os.path.join(args.pretrained_model_dir, dataset)
                checkpoint_file = os.path.join(checkpoint_dir, args.pretrained_from+".pth.tar")
