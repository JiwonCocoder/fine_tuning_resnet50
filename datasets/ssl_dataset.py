...
class SSL_Dataset:
    ...
    and return BasicDataset: torch.utils_from_git.data.Dataset (see datasets.dataset.py)
    """
    def __init__(self,
                 name='MLCC',
                 learning_type='semi',
        ...
        self.transform = get_transform(name, learning_type, train)
        print("------------data-------------")
        print("dataset_name:", self.name)
        print("transform:", self.transform.transforms)
        ...
        if hasattr(torchvision.datasets, self.name.upper()):
            ...
            for i, listname in enumerate(labeled_dir_list):
                ...
                for filename in file_names:
                    ...
                    data_pil.append(np.asarray(pil_loader(os.path.join(file_directory, filename))))
                    target_pil.append(i)
            random.Random(0).shuffle(data_pil)
            random.Random(0).shuffle(target_pil)
            # random.seed(0)
            # shuffle_data = random.shuffle(data)
            # shuffle_target = random.shuffle(target)
            data = np.array(data_pil)
            targets = np.array(target_pil)
    ...
    def get_ssl_dset(self, num_labels, index=None, include_lb_to_ulb=True,
        ...
        """
        print("get_dset_params:", use_strong_transform, strong_transform, onehot)
        print("include_lb_to_ulb: If True, consistency regularization is also computed for the labeled data.", include_lb_to_ulb)
